"""L01_AS008_load — load Shaikh Appendix 6.8 columns for AS008 (GPIM Variant - Interwar Adjustment Multiplier).
...
NIPA / BEA FA / IRS / Census components are documented in AS008_EPR.md.

Source map (subseries_id -> (Appendix table, variable, scale)):
  AS008-A <- Appendix 6.8.II5 / variable 'Adj. Ratio'

Units: dimensionless_ratio_1925eq1
Book year range: [1925, 1947]
"""
...
SOURCE_MAP = {'AS008-A': ['II5', 'Adj. Ratio', 1.0]}
...
def run() -> dict:
    ...
    for sub_id, (table, var, scale) in SOURCE_MAP.items():
        ...
        df["units"] = "dimensionless_ratio_1925eq1"
