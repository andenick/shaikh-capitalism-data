"""P02_AS009_construct — construct AS009 from raw parquet (pass-through with cleaning).

Reads Technical/data/raw/AS009_raw.parquet (one row per (year, subseries_id))
and writes Technical/data/processed/AS009.parquet with the canonical
``year, value, subseries_id, source_id, units`` columns.

Construction: the Shaikh Appendix 6.8 chopped tables already contain the
finished AS009 columns; the loader applied unit normalization at fetch time
