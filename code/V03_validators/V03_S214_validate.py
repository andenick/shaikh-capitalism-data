"""V03_S214_validate - book period data_unavailable; PASS with explicit marker."""
...
def run() -> dict:
    ...
    row = {
        ...
        "reason": "anwarshaikhecon.org App 7.2 / OECD ISDB 1994 not in SalvagedInputs; per anu-framework no-fabrication rule, book values are not synthesized.",
        "post_book_rows": int(len(df)),
        "post_book_range": [int(df["year"].min()), int(df["year"].max())],
        "remediation": "When App 7.2 replica is added to SalvagedInputs, re-run L01 to populate 1960-1989.",
