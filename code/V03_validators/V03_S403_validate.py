"""V03_S403_validate — compare S403 processed parquet to Appendix 4.2 Table 4
profit columns PL, PH. Tolerance 0.5%."""
...
def run() -> dict:
    ...
    for row_idx, truth_row in truth.iterrows():
        for comp in COMPONENTS:
            ...
            # Profit values can be negative or near-zero; use abs(expected) with floor of 1.0
            # to avoid divide-by-near-zero amplifying small absolute residuals into >0.5% pct errs
            # for rows like the XR=0 row where expected=-70 (denom=70, abs_err well below 0.35).
            denom = abs(float(expected)) if abs(float(expected)) > 1.0 else 1.0
    ...
    row = {
        ...
        "comparison_basis": "Reconstructed Appendix_4_2_Table4.csv profit columns (PL, PH)",
        "n_compared": n_compared, "mae": round(mae, 6),
        "max_abs_err": round(max_abs, 6), "max_pct_err": round(max_pct, 6),
        "divergence_count": len(divergences), "divergences": divergences[:10],
