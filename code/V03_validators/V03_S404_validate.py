"""V03_S404_validate — data_unavailable. Returns PASS_DATA_UNAVAILABLE."""
...
def run() -> dict:
    # Verify DPR documents data_unavailable status (anti-degradation guard).
    dpr_ok = DPR.exists() and "data_unavailable" in DPR.read_text(encoding="utf-8")
    row = {
        "status": "PASS_DATA_UNAVAILABLE",
        "tolerance_pct": None,
        "content_type": "derived",
        "comparison_basis": "n/a — Inman 1995 paywalled, figures-only, no underlying data",
        ...
        "remediation_path": "Phase 9 may pursue authorized library access or "
                            "Decision-Log-approved figure digitization. Until then "
                            "the series is intentionally empty in the processed pipeline.",
