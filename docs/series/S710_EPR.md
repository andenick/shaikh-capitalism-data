...
This series is **formula**: it is the algebraic transform `dev_i,t = level_i,t − aggregate_t`. Therefore extension must re-fetch the underlying S706 components (BEA (US Bureau of Economic Analysis) primaries) and re-compute the deviation; no growth-rate splice is permissible.
...
```
INPUTS
  S706's processed parquet (processed data file)
...
CD2 (the predecessor build of this dataset) used the same xlsx for `S037`. RSCD's Phase 5 deliverable should match CD2 at MAE = 0 on the book period.

## Notation (plain-language key)

Short forms used above, in plain language (this record is a downloadable external artifact):

- **S###** — series identifiers in this project (e.g. S706, S710).
