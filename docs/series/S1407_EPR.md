# S1407 — Extension Provenance Record

**Chapter 14 · Authored 2026-05-18**

## Classification
`time_series` · `direct`. Extension via S1401/S1402/S1406 components.

## Method (both variants emitted)
- **direct_refit**: fit Phillips form to extended GRWAGEHP100 on each era
  window
- **algebraic_decomposition**: S1405-era-fit + HP100 (the Hodrick-Prescott filter, smoothing parameter lambda=100) of productivity growth
  per Eq. 14.18

Both reported in `S1407_phillips_fits.json` sidecar.

## Concept policing
Inherited from S1406 (real wage construction reuses same per-full-time-equivalent (FTE) inputs).

## No-Proxy / No-Synthetic disclosures
Inherits from S1401, S1402, S1406.

## Divergence from the predecessor build
Minor expected.
