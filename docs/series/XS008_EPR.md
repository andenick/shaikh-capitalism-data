...
For XS008, the round-trip validation reads the raw values from the Appendix 6.8 workbook and confirms bit-for-bit reproduction of the published series for the book period 1925-1947. A worked-example year (typically 2009 per book p. 842, or 2011 per Shaikh's last published vintage) verifies headline values.
...
Round-trip parity with an earlier replication is expected within tolerance for the book period. The comparison against the earlier replication's per-series output is informational when available.
