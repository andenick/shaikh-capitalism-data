...
**Authored**: 2026-05-18
**Related**: `S301_DPR.md`, research dossier

---

## 1. Classification

S301 is a `theoretical` series (this classification was assigned during the data-intake review and ratified in `CH3_ADEQUACY_REPORT.json`). It is an analytic curve derived from Shaikh's equations (3.5)-(3.11) under Case I, plotted over a synthetic income grid. There is no time dimension.

## 2. Method

**Extension method**: `none`. The series is regenerated from the published equations each time the data-loading step runs. There is nothing to splice forward.
...
|---|---|---|
| Calibration choice (x1min(y), c) departs from book's apparent figure | The validation step's shape/bounds checker flags out-of-range values | Re-calibrate; document new choice in the Data Provenance Record (DPR) |
| Numeric overflow at y=0 (square-root singularity) | The data-loading step filters y<=0.5 | Trim grid to y in [0.5, 60]; documented as a known artefact |
| Disagreement with empirical analogs (S306/S307) | Out of scope for the validation step of S301; tracked in later chapter-level analysis | Not a S301 failure — the theoretical claim is compatibility with shape, not exact agreement with the 1904 cross-section |

## 7. Predecessor divergence pre-disclosure

Not applicable. Predecessor series: none (first constructed in this dataset). S301 was not part of an earlier reconstruction of this project (internally called CD2); that earlier reconstruction did not contain this chapter's series. The validation step therefore omits the predecessor-comparison block for this series.
...
This series will be re-evaluated each time the data-loading step runs, from the same equations — that is the only meaningful sense in which it is "kept current."
