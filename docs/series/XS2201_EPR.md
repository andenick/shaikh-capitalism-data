...
AGI distribution` or `top-10% income share (Piketty-Saez)` because:
...
- Methodology continuity: paper-window 2002-2016 parameters were MLE-fit
  ...
  (per Anu Framework No Lazy Splices rule, since these five parameters
  are jointly estimated, not directly observed).
...
is NOT a proxy substitution forbidden by the No-Proxy rule because
