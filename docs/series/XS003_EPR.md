...
4. Table 7.11 line ids are resolved by a stub-label resolver that uses the BEA-canonical stub labels rather than the 2011-vintage line numbers; this survives the 2013 and 2018 revisions to financial services indirectly measured (FISIM).
5. Recompute BusImpIntAdj for each year and propagate into sectoral profit rates.

## Worked Example

For XS003, the round-trip validation reads the raw series from the Appendix 6.8 workbook and confirms bit-for-bit reproduction of the published series for the book period 1947-2011. A worked-example year (typically 2009 per book p. 842, or 2011 per Shaikh's last published vintage) verifies headline values.
...
| FISIM revision to NIPA Table 7.11 line numbers (affects XS003) | The Table 7.11 line resolver falls back to the nearest pinned vintage with a logged warning | Lines are re-mapped by their published stub labels; the vintage used is logged |
