...
- **algebraic_decomposition**: S1405-era-fit + HP100 (the Hodrick-Prescott filter, smoothing parameter lambda=100) of inflation +
...
## Concept policing
Inherited from S1406 (nominal wage w = EC*100/FEE (FEE = full-time-equivalent employees) uses same per-full-time-equivalent (FTE)
convention).
