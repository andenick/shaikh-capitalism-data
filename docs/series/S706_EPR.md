...
CD2 (the predecessor build of this dataset) has a per-series CSV for `S036` that is the closest legacy comparison. CD2 used the same Shaikh Appendix 7.2 source for the book period and applied its own end-to-end re-run for the 2006–2024 extension. RSCD's Phase 5 deliverable does not extend; in this wave RSCD's book-period values should agree with CD2 at MAE = 0 (modulo presentation choices on the All-Private aggregate column).
