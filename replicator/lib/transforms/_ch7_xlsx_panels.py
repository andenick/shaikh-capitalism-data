...
# classification_vintage (Decision 0014 / SI-3): the ch7 30-industry ROP/IROP panels
# (S705/S706/S709/S710) are NAICS-native from 1997, tracked across the NAICS revision
# chain — a single classification basis, NOT a SIC/NAICS splice. The known tag is
# NAICS_1997ff (canonical token NAICS_1997). assert_panel_vintage is the ch7-side
# loader assertion (the CH9-F4-analogous guard for the industry panel).
PANEL_CLASSIFICATION_VINTAGE = "NAICS_1997"


def assert_panel_vintage(claimed: str, expected: str = PANEL_CLASSIFICATION_VINTAGE) -> str:
    """Assert a ch7 panel loader's declared classification_vintage matches the known one.

    Raises AssertionError on mismatch (a wrong tag on the NAICS-native industry panel
    would be a latent classification error). Returns the validated tag.
    """
    if claimed != expected:
        raise AssertionError(
            f"ch7 industry panel classification_vintage {claimed!r} != known {expected!r} "
            f"(the 30-industry ROP/IROP panel is NAICS-native from 1997)"
        )
    return claimed
