...
def fred_observations(
    ...
    """Fetch FRED / ALFRED series observations (the shared pinned-fetch path).

    Parameters
    ----------
    series_id : FRED series identifier (e.g. 'INDPRO').
    frequency : 'a' annual, 'q' quarterly, 'm' monthly, or ``None`` for the
        series' NATIVE frequency (no FRED-side aggregation — the drop-in for the
        legacy ``fred_csv_observations`` transport used by the Ch10 extension
        loaders, which aggregate client-side).
    aggregation_method : 'avg' (mean), 'sum', 'eop' end-of-period. Ignored when
        ``frequency is None`` (native).
    observation_start, observation_end : YYYY-MM-DD strings (optional).
    realtime_start, realtime_end : ALFRED vintage window (YYYY-MM-DD). When both
        are set to the SAME date the response is the vintage of the data AS IT
        EXISTED ON THAT DATE (SI-1 reproducibility pinning; see
        ``config/VINTAGE_MANIFEST.json`` and ``utils.vintage_manifest``). When
        omitted, FRED returns the LATEST vintage (non-reproducible).
    ...
    # realtime params are part of the cache key so a pinned vintage caches
    # separately from a latest-vintage fetch (reproducibility).
    query = {
        "series_id": series_id,
        "frequency": frequency,
        "aggregation_method": aggregation_method if frequency is not None else None,
    ...
    S00_cache.put("fred", query, df, extra_meta={
        "endpoint": FRED_BASE, "series_id": series_id,
        "realtime_start": realtime_start, "realtime_end": realtime_end,
        "vintage_pinned": bool(realtime_start and realtime_end),
    })
    return df
...
def census_ft900_annual_balance(
    ...
    partner : Census country code as a 4-digit string with trailing zero
              (e.g. ``"5700"`` for China, ``"0004"`` for the World total).
              The World total is published at the same per-country balance
              page (``balance/c0004.html``) as every other partner, so it is
              parsed identically. Legacy callers may pass ``"world"``; it is
              normalized to ``"0004"`` (the shipped data uses the c0004
              country-balance page, NOT FT900 Exhibit 1).
    ...
    """
    # World total is Census country code 0004, published at the same
    # per-country balance page as any partner. Normalize the legacy "world"
    # alias to "0004" BEFORE the cache key so the source id / page never
    # falls back to the FT900 Exhibit 1 text file (the shipped XS2301 data is
    # on c0004; item A1, 2026-07-10).
    if partner == "world":
        partner = "0004"
    ...
    rows: list[dict] = []
    # Per-country balance page — parse with pd.read_html. This path serves EVERY
    # partner including the World total (country code 0004): balance/c0004.html
    # has the same per-country monthly-table layout as any partner page.
    from io import StringIO
    try:
