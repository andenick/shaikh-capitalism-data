"""V03_ES2101_validate — verify ES2101 matches reconstructed summary stats."""
...
def run() -> dict:
    ...
    for _, t in truth.iterrows():
        year = int(t["year"])
        stat = str(t["statistic"])
        expected = float(t["value"])
        sub_id = f"{SERIES_ID}-{stat}"
    ...
    row = {
        ...
        "comparison_basis": "Reconstructed ES2101_summary_statistics.csv (verbatim Section 5 p. 272)",
        "n_compared": n,
        "mae": round(mae, 6),
