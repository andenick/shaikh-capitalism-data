"""V03_ES2304_validate — verify processed ES2304 matches the reconstructed CSV."""
...
def run() -> dict:
    ...
    n = 0
    # Match by (estimate_year, study) -> (year, country_key prefix)
    for _, t in truth.iterrows():
        yr = int(t["estimate_year"])
        study = str(t["study"])
        expected = float(t["misalignment_pct"])
        # Country key uses study + "_" + year
        ck = f"{study}_{yr}"
    ...
    row = {
        ...
        "comparison_basis": "Reconstructed ES2304_literature_compilation.csv (verbatim named endpoints from paper note 17)",
        "n_compared": n,
        "mae": round(mae, 6),
