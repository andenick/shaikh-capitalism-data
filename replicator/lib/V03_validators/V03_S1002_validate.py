"""V03_S1002_validate — compare S1002-A vs USLR['iblong'] and S1002-B vs USLR['USWPI']."""
...
PROCESSED = DATA_PROCESSED / "S1002.parquet"
...
def _load_truth() -> pd.DataFrame:
    ...
    return df[["Year", "iblong", "USWPI"]].rename(columns={"Year": "year"})
...
def _update_report(row: dict) -> None:
    ...
    rpt.setdefault("series", {})["S1002"] = row
...
def run() -> dict:
    ...
    for sub_id, col in [("S1002-A", "iblong"), ("S1002-B", "USWPI")]:
        a = actual[actual["subseries_id"] == sub_id][["year", "value"]]
        m = a.merge(truth[["year", col]].rename(columns={col: "expected"}).dropna(subset=["expected"]),
                    on="year", how="inner")
        if not len(m):
            per_sub[sub_id] = {"n": 0, "note": "no overlap"}
            continue
        ...
        per_sub[sub_id] = {"n": int(len(m)), "mae": round(mae, 6),
                           "max_pct_err": round(max_pct, 6), "div_years": div[:20],
                           "div_count": len(div)}
        div_years_total += div
        n_total += len(m)
