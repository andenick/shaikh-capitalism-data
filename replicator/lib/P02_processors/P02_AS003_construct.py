"""P02_AS003_construct — construct AS003 from raw parquet (pass-through with cleaning).

Reads Technical/data/raw/AS003_raw.parquet (one row per (year, subseries_id))
and writes Technical/data/processed/AS003.parquet with the canonical
``year, value, subseries_id, source_id, units`` columns.

Construction: the Shaikh Appendix 6.8 chopped tables already contain the
finished AS003 columns; the loader applied unit normalization at fetch time
