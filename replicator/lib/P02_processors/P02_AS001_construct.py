"""P02_AS001_construct — construct AS001 from raw parquet (pass-through with cleaning).

Reads Technical/data/raw/AS001_raw.parquet (one row per (year, subseries_id))
and writes Technical/data/processed/AS001.parquet with the canonical
``year, value, subseries_id, source_id, units`` columns.

Construction: the Shaikh Appendix 6.8 chopped tables already contain the
finished AS001 columns; the loader applied unit normalization at fetch time
