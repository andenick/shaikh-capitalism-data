"""P02_S603_construct — construct S603 from raw parquet (pass-through with cleaning).

Reads Technical/data/raw/S603_raw.parquet (one row per (year, subseries_id))
and writes Technical/data/processed/S603.parquet with the canonical
``year, value, subseries_id, source_id, units`` columns.

Construction: the Shaikh Appendix 6.8 chopped tables already contain the
finished S603 columns; the loader applied unit normalization at fetch time
