...
def validate_against_panel(
    ...
    column_relabel: optional {old_col: new_col} applied to the TRUTH xlsx columns
        before building the (year, industry) key. Used by S801 (F-T2-01) where the
        frozen digitized xlsx has the two industry columns transposed vs the source
        figure; the loader corrects the labels, so the truth must be relabeled the
        same way for the round-trip to certify the corrected mapping. Default None =
        no change (every other ch7 validator is byte-identical in behavior).
    """
    ...
    if column_relabel:
        # Correct a transposed-label source (F-T2-01, S801) before keying.
        truth = truth.rename(columns=column_relabel)
